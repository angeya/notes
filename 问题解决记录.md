...
因为可能起的微服务实例很多，这时候应该使用分布式锁，比如redis或者zookeeper
...
>  注意：放宽字符校验虽然方便调试，但如果你的服务是对外开放的，这可能带来安全风险，需谨慎使用。

#### SpringCloudBus监听不到事件

出现这种情况的原因有很多，这里记录一种反序列化失败导致监听不到的情况。
...
```
springframework.cloud.bus.BusConsumer : Received remote event from bus: [UnknownRemoteApplicationEvent@33ec467c id = '67017f8c-c1ea-400f-b2f5-b1106e52e544', originService = '', destinationService = 'unknown', typeInfo = 'SystemNoticeEvent', payload =xxx
```

说明消息事件对象反序列化失败，降级为`UnknownRemoteApplicationEvent`对象了，通常是Jackson没有加载类作为反序列化候选导致的，它默认只加载和启动类统一包名下的类。

这时候可以在启动类中增加`@RemoteApplicationEventScan`注解指定事件类。

或者通过在配置类中灵活给Jackson注册类。代码示例如下：

```java
@Configuration
public class BusEventConfiguration {
    @Bean
    public Module customBusEventsModule() {
        // 通过增量方式向ObjectMapper注册子类型，否则启动类包外的的事件类可能会被扫描反序列化为UnknownRemoteApplicationEvent
        SimpleModule module = new SimpleModule("CustomBusEvents");
        // 需要注册子类型可以继续加在后面
        module.registerSubtypes(SystemNoticeEvent.class);
        return module;
    }
}
```
